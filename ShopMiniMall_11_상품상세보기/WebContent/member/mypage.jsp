...
<script type="text/javascript">
	$(document).ready(function(){
		...
		$("form").on("submit", function(event) {
			...
			if(userid.length == 0){ //id. 비번이 0인 경우 
				alert("userid 필수");		// 에러창 띄우고
				$("#userid").focus();
				event.preventDefault();	//전송금지
			}else if (condition) {
				alert("passwd 필수");
				$("#passwd").focus();
		...
		$("#passwd2").on("keyup", function() {
			var passwd = $("#passwd").val();
			var mesg = "비번 불일치";
			if(passwd == $(this).val()){	//키 이벤트 발생시 패스워드 일치여부 검사 
				mesg = "비번 일치";
			}
			console.log(passwd);
			$("#result2").text(mesg);
...
<form action="MemberUpdateServlet" method="post">
	...
	*아이디: <%= userid %><br> 
	...
	<br> 
	이름: <%= username %><br> 
	<input type="text" name="post" id="sample4_postcode" value="<%= post %>">
	<input type="button" onclick="sample4_execDaumPostcode()" value="우편번호 찾기"><br>
	<input type="text" name="addr1" id="sample4_roadAddress" value="<%= addr1 %>">
	<input type="text" name="addr2" id="sample4_jibunAddress" value="<%= addr2 %>">
